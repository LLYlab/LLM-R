...
两套主题，**跟随系统**或显式选一个；SWF 自带的界面跟着一起切。

<img src="docs/screenshots/light.png" width="900" alt="亮色模式">

暗色是底色（一个 runtime 控制台该长这样），亮色是为白天和投影准备的。
两边共用同一套令牌——强调色的各种透明度用 `color-mix` 从 `--accent` 派生，
**不各写一遍**，否则每加一档透明度都要在两边各写一次，迟早漏。

> SWF 自带的页面（`ui/*.html`）也长在这套令牌上，所以它们**不用改一行**就跟着换主题。
> 主题值由**服务器**注入到页面的首帧（`?__theme=`）——iframe 是 sandbox 的、读不到
> localStorage，不能指望它自己去查。
